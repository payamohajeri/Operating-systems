    ...
    if(child_pid == 0) {
        ...
        } if (piping)
        {
            ...
            if (pid == 0)
            {
                ...
                dup2(pipefd[0], 0);
                close(pipefd[1]);
                // char * pipe_arguments[10];
                // char arg2[20];
                // char pipe_command[20];
                // int count=0;
                // int startindex=0;
                ...
                //     if(pipecommand[i]==' ' && !commandflag)
                //     {
                //         for(int j=0; j<i; j++)
                //         {
                //             pipe_command[j]=pipecommand[j];
                //         }
                //         commandflag = true;
                //         pipe_command[i]='\0';
                //         startindex=i;
                //     } else if (pipecommand[i]==' ') {
                //         for(int j=startindex; j<i; j++)
                //         {
                //             arg2[j]=pipecommand[j];
                //         }
                //         arg2[i]='\0';
                //         pipe_arguments[count] = arg2;
                //         count++;
                //         startindex=i;
                //     }
                // }
                // pipe_arguments[count] = NULL;
                //printf("child, %s \n", pipecommand );
                //printf("pid %d\n", pid);
                //execvp(pipe_command, pipe_arguments);
                char * pipe_arguments[] = {pipecommand, NULL};
                execvp(pipecommand, pipe_arguments);
                dup2(fd, 0);
                close(fd);
            } else {
                //printf("parent, %s \n", argv[0] );
